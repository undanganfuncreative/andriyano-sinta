(() = > {
    ...
    y && p && (p.innerHTML = y);
    var f = document.getElementById("btnMusic"),
    ...
    playMusic = function(e) {
        v && (e ? (v.play(), w = !0) : (w ? v.pause() : v.play(), w = !w)), h && SC.Widget.Events.READY && (w ? (h.pause(), w = !1) : (h.play(), w = !0)), f && (w ? f.classList.add("playing") : f.classList.remove("playing"))
    }, v && (v.onplaying = function() {
        w = !0, f.classList.add("playing")
    }, v.onpause = function() {
        w = !1, f.classList.remove("playing")
    }), showGift = function(e) {
        for (var t = 0; t < E.length; t++) t != e && (b[t].style.display = "none");
    ...
    showLightbox = function(e) {
        M.classList.add("show"), H.innerHTML = '<div class="lightbox-inner"><img src="'.concat(S[e].src, '"></div>'), I.dataset.index = e, _.dataset.index = e, ge(), window.removeEventListener(ce[V].down, ye, !1)
    }, I.onclick = function() {
        var e = parseInt(I.dataset.index) + 1;
        e >= S.length && (e = 0), showLightbox(e)
    }, _.onclick = function() {
        var e = parseInt(_.dataset.index) - 1; - 1 == e && (e = S.length - 1), showLightbox(e)
    ...
    showModal = function(e) {
        C.classList.add("modal-open"), A.classList.add("show"), A.style = "display: block;", e.classList.add("show"), e.style = "display: block;", ge(), window.removeEventListener(ce[V].down, ye, !1)
    }, closeModal = function(e) {
        C.classList.remove("modal-open"), A.classList.remove("show"), A.style = "display: none;", e.classList.remove("show"), e.style = "display: none;", window.addEventListener(ce[V].down, ye, !1)
    };
    ...
    displayCountdown = function(e) {
        var t = new Date(e.dataset.datetime).getTime(),
            ...
            i = setInterval((function() {
                ...
                n.innerHTML = r, o.innerHTML = c, a.innerHTML = d, l.innerHTML = u, s < 0 && (clearInterval(i), n.innerHTML = "00", o.innerHTML = "00", a.innerHTML = "00", l.innerHTML = "00")
            }), 1e3)
    };
    for (var W = 0; W < N.length; W++) displayCountdown(N[W]);
    for (var D = document.getElementsByClassName("btn-rsvp"), F = null !== (e = document.querySelector(".rsvp-placeholder")) && void 0 !== e ? e : null, O = null !== (t = document.querySelector(".rsvp-form")) && void 0 !== t ? t : null, Y = 0; Y < D.length; Y++) F ? D[Y].style.display = "none" : D[Y].onclick = function() {
        showModal(rsvpModal)
    };
    ...
    setTimeout((function() {
        z && P && null == U ? (z.style.display = "inherit", P.innerHTML = "", P.appendChild(z), j.style.display = "none") : G && null == P ? (R.innerHTML = "", j.style.display = "flex") : (z && (z.style.display = "none"), j.style.display = "none")
    }), 300);
    for (var X = document.getElementsByClassName("account-number"), Z = 0; Z < X.length; Z++) X[Z].style.display = "inline", X[Z].innerHTML && ("id" == c ? X[Z].insertAdjacentHTML("afterend", "<button type='button' class='btn btn-sm btn-primary ml-2 mb-1 animate__animated animate__zoomIn animate__slow delay-5' data-text='".concat(X[Z].innerText, "' onclick='copyText(event)' style='font-family: sans-serif; border-radius: 4px; line-height: 1'>Salin</button>")) : X[Z].insertAdjacentHTML("afterend", "<button type='button' class='btn btn-sm btn-primary ml-2 mb-1 animate__animated animate__zoomIn animate__slow delay-5' data-text='".concat(X[Z].innerText, "' onclick='copyText(event)' style='font-family: sans-serif; border-radius: 4px; line-height: 1'>Copy</button>")));
    for (var J = document.getElementsByClassName("copy-address"), K = 0; K < J.length; K++) J[K].innerHTML && ("id" == c ? J[K].insertAdjacentHTML("afterend", "<button type='button' class='btn btn-sm btn-primary mt-2 animate__animated animate__fadeInUp animate__slow delay-5' data-text='".concat(J[K].innerText, "' onclick='copyText(event)' style='font-family: sans-serif; border-radius: 4px;'>Salin Alamat</button>")) : J[K].insertAdjacentHTML("afterend", "<button type='button' class='btn btn-sm btn-primary mt-2 animate__animated animate__fadeInUp animate__slow delay-5' data-text='".concat(J[K].innerText, "' onclick='copyText(event)' style='font-family: sans-serif; border-radius: 4px;'>Copy to Clipboard</button>")));
    copyText = function(e) {
        ...
        e.target.innerHTML = "id" == c ? "Disalin" : "Copied", setTimeout((function() {
    ...
    var Q = function() {
        ...
        e.style.height = "".concat(s, "px"), r.style.height = "".concat(s, "px"), t.style.transform = "scale(".concat(i, ") translate(0px,0px)")
    };
    ...
    var ee = document.querySelectorAll(".satumomen_slide"),
        ...
        ie = function() {
            Array.from(ee).forEach((function(e) {
                ...
                e.style.maxWidth = "".concat(ae, "px"), e.classList.remove("active"), e.onclick = function(t) {
                    t.cancelable && t.preventDefault(), le = Array.from(te).indexOf(e), re(le)
                }
            })), ee[le].style.display = "", te[le].classList.add("active")
        };
    ie();
    var se = function() {
    ...
    pe.addEventListener("click", (function(e) {
        e.cancelable && e.preventDefault(), fe ? ge() : fe || he ? ve() : we()
    }), !1), openFullScreen = function() {
